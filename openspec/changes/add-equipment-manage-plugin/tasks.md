...
- [x] 1.2 幂等`SQL`（两表、索引、字典 seed）、卸载与`mock-data`。
- [x] 1.3 `backend/api/equipment`与`backend/api/maintenance`的`RESTful DTO`。
- [x] 1.4 建表并生成`dao/ctrl`。

## 2. 后端实现

- [x] 2.1 设备服务：租户过滤`CRUD`、编号唯一校验、删除引用保护、创建人批量装配。
- [x] 2.2 维护记录服务：设备存在校验、列表设备名称批量装配、`bizerr`错误码。
- [x] 2.3 控制器与路由绑定，契约测试与单测通过。

## 3. 前端实现

- [x] 3.1 设备登记页（列表/筛选/新增编辑弹窗/删除确认）。
- [x] 3.2 维护记录页（列表按设备筛选、登记弹窗设备候选联动）。
