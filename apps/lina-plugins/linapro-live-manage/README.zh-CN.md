...
| 直播日程日历订阅 | 免登录的公开 ICS 订阅接口，观众在系统日历订阅一次即可自动同步公开直播日程。 |
...
| `GET /x/linapro-live-manage/api/v1/subscribe?roomCode={roomCode}&tenantId={tenantId}` | 公开日历订阅接口，返回 RFC 5545 ICS 日历流。 |
...
订阅接口覆盖最近 7 天至未来 90 天内的公开直播，按直播日期升序输出：

- 每场公开直播一个`VEVENT`，`UID`以直播记录为稳定键，重复刷新与重新订阅不产生重复条目。
- 未开始且未录入开播时间的直播以`liveDate`全天事件表达，不编造具体时间；其余条目输出 UTC 起止时间。
- 预告态 H5 页提供"添加到日历"入口；管理端直播间列表提供"日历订阅"弹窗展示订阅链接。
- 直播间不存在、无公开直播或租户校验失败统一返回合法空日历（HTTP 200），不泄露房间存在性。
