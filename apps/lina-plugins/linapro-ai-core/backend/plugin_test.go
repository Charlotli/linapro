...
// TestSourcePluginRegistersAITextProvider verifies the plugin declares its
// text AI provider factory through the pluginhost provider declarations.
func TestSourcePluginRegistersAITextProvider(t *testing.T) {
	...
	if definition.GetAITextProviderFactory() == nil {
		t.Fatal("registered source plugin must declare a text AI provider factory")
	}
}
