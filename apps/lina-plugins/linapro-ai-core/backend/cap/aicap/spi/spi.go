...
// owner capability. It shares stable AI method and contract identifiers with
// guest SDKs and the host while all provider implementations and business
// storage remain inside the linapro-ai-core backend internals. Provider
// registration goes through pluginhost Declarations.Providers().ProvideAIText.
package spi
...
const (
	...
	// PublishedMethods until a real provider path is wired.
...
// PublishedMethods returns the AI method names currently published by the
// owner. Only methods with a real runtime path are published so authorization
// and catalog projections match actual host behavior. Multimodal DTO packages
// remain available for source consumption and will be added here when their
// provider SPI is wired.
func PublishedMethods() []string {
	return []string{
		MethodTextGenerate,
		MethodTextStatusGet,
