// This file keeps linapro-oa-approval API DTO boundary checks inside the plugin module.
...
import (
	...
	approvalv1 "lina-plugin-linapro-oa-approval/backend/api/approval/v1"
	flowv1 "lina-plugin-linapro-oa-approval/backend/api/flow/v1"
)

// TestOAApprovalAPIsDoNotDependOnGeneratedEntities ensures public API contracts do not import database entities.
func TestOAApprovalAPIsDoNotDependOnGeneratedEntities(t *testing.T) {
	assertNoGeneratedEntityImports(t, oaApprovalPluginRoot(t))
}

// TestOAApprovalAPIDTOsDoNotUseEntityNaming ensures response DTOs use API-oriented names.
func TestOAApprovalAPIDTOsDoNotUseEntityNaming(t *testing.T) {
	assertNoEntityTypeNames(t, oaApprovalPluginRoot(t))
}

// TestOAApprovalAPIDTOFilePlacement keeps shared DTOs in the API package main source file.
func TestOAApprovalAPIDTOFilePlacement(t *testing.T) {
	assertTypeDeclaredInFile(t, oaApprovalPluginRoot(t), map[string]string{
		"flow/v1/FlowItem":              "flow.go",
		"approval/v1/RequestItem":       "approval.go",
		"approval/v1/RequestRecordItem": "approval.go",
		"approval/v1/RequestNodeItem":   "approval.go",
	})
}

// TestOAApprovalAPIDTOFilesAvoidLegacyNames rejects legacy DTO-only file naming.
func TestOAApprovalAPIDTOFilesAvoidLegacyNames(t *testing.T) {
	assertNoLegacyDTOFiles(t, oaApprovalPluginRoot(t))
}

// TestOAApprovalResponseDTOsHideInternalFields verifies implementation fields stay out of responses.
func TestOAApprovalResponseDTOsHideInternalFields(t *testing.T) {
	internalFields := internalResponseFields()
	assertJSONFieldsAbsent(t, "flow.FlowItem", reflect.TypeOf(flowv1.FlowItem{}), internalFields)
	assertJSONFieldsAbsent(t, "approval.RequestItem", reflect.TypeOf(approvalv1.RequestItem{}), internalFields)
}

// TestOAApprovalAPIDocI18NDoesNotReferenceRemovedDTOFields keeps apidoc translations aligned with DTOs.
func TestOAApprovalAPIDocI18NDoesNotReferenceRemovedDTOFields(t *testing.T) {
	assertAPIDocI18NExcludesTokens(t, oaApprovalPluginRoot(t), []string{
		"RequestEntity",
		"FlowEntity",
		"nodeSnapshot",
		"deletedAt",
...
// oaApprovalPluginRoot returns the plugin root directory for path-based contract checks.
func oaApprovalPluginRoot(t *testing.T) string {
	t.Helper()
